...
Public Class InvAccounts
    ...
    Private Sub cmbfill()
        ...
        Dim tmpt As String
        cmd.Connection = Conn
        cmd.CommandText = "Select name from Invaccounts where mode ='R' group by name order by name"
        da.SelectCommand = cmd
        da.Fill(ds, "Patient")
        cbPat.Tag = "Y"
        tmpt = cbPat.Text
        cbPat.DataSource = ds.Tables(0)
        cbPat.DisplayMember = "Name"
        cbPat.Text = tmpt
        cbPat.Tag = ""
    ...
    Private Sub GetTotal()
        ...
        cmd.CommandText = "Select round(isnull(sum(amt-adjamt),0),2) as [Balance]  from Invaccounts where mode ='R'  "
    ...
    Private Sub populate()
        ...
        Try
            ...
            cmd.Connection = Conn
            cmd.CommandText = "Select billno  ,convert(datetime,billdt,103) ," & _
            "amt  ,adjamt  ,amt-adjamt,id,par,remarks from Invaccounts where mode ='R' and name = """ + _
            cbPat.Text + """" & sqltxt & " order by billdt,id"
            da.SelectCommand = cmd
            da.Fill(dt)
            ...
            For i = 0 To dt.Rows.Count - 1
                grdacc.Rows.Add()
                grdacc.Item(0, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(0)
                grdacc.Item(1, grdacc.Rows.Count - 1).Value = Format(dt.Rows(i).Item(1), DateFormat)
                grdacc.Item(2, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(2)
                grdacc.Item(3, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(3)
                grdacc.Item(4, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(4)
                grdacc.Item(5, grdacc.Rows.Count - 1).Value = "" 'dt.Rows(i).Item(5)
                grdacc.Item(6, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(5)
                grdacc.Item(7, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(6)
                grdacc.Item(8, grdacc.Rows.Count - 1).Value = dt.Rows(i).Item(7)
                    ...
                    Case 5
                        ...
                        For I = grdacc.CurrentCell.RowIndex To grdacc.Rows.Count - 1
                            ...
                            If Val(grdacc.Item(4, I).Value & "") > 0 Then
                                ...
                                End If
                                cmd.Connection = Conn
                                cmd.CommandText = "update invAccounts set remarks='Cash Paid on " & SysDt & "',adjamt=(adjamt-" & Val(grdacc.Item(5, I).Value & "") & "+" & ADJVAL & ") where id=" & Val(grdacc.Item(6, I).Value & "")
                                cmd.ExecuteNonQuery()
    ...
    Private Sub grdacc_DataError(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewDataErrorEventArgs) Handles grdacc.DataError
    ...
    Private Sub grdacc_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles grdacc.KeyDown
        Try
            ...
            If e.KeyCode = Keys.Delete Then
                Dim cmd As New OleDb.OleDbCommand
                cmd.Connection = Conn
                If MsgBox("Want to delete selected entries ?", vbYesNo + vbQuestion + vbDefaultButton2) = vbNo Then Exit Sub
                cmd.CommandText = "delete from invAccounts where id=" & Val(grdacc.Item(6, grdacc.CurrentRow.Index).Value & "")
                grdacc.Rows.Remove(grdacc.CurrentRow)
                cmd.ExecuteNonQuery()
    ...
    Private Sub cbPat_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbPat.SelectedValueChanged

    End Sub

    Private Sub cbPat_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbPat.TextChanged
        If cbPat.Tag = "Y" Then Exit Sub
        populate()
        GetTotal()
    End Sub
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Comprint.Click
        ...
        cmd.CommandText = "GetInvReceivableByName"
        cmd.Parameters.Add("@name", OleDb.OleDbType.VarChar).Value = cbPat.Text
         
        da.SelectCommand = cmd
        da.Fill(dtab)

        Dim FRMRPT As New Reports1
        Dim RPT As New RptInvReceivableNamewise

        RPT.SetDataSource(dtab)

        RPT.SetParameterValue("Head1", DeScriptRS(Firm.Name))
        RPT.SetParameterValue("HEAD2", DeScriptRS(Firm.Place))
        RPT.SetParameterValue("HEAD4", "Account Receivable")
