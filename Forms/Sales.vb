...
Public Class Salesinv
        ...
        Try
            ...
            If dt.Rows.Count > 0S Then
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    dgInv.Item(19, dgInv.Rows.Count - 1).Value = Round(dt.Rows(i).Item("Pdis"), 2)
    ...
    Private Sub GetSeries()
        ...
        Try
            ...
            ElseIf cbType.SelectedValue = 24 Then
                ...
                cmd.Connection = Conn
                Try
                    cmd.CommandText = "Select TransDoc from settings "
                    cbDoc.Text = (cmd.ExecuteScalar & "")
            ...
            cmd.CommandText = "select isnull(sum(netamt),0) from invtotal where invtotal.type='" & cbType.SelectedValue & "' and invdt='" & Format(SysDt, "yyyyMMdd") & "'"
    ...
    Private Sub ClearForm(Optional ByVal flg As Boolean = False)
        Dim DA As New OleDb.OleDbDataAdapter
        'Dim dr1 As New OleDb.OleDbDataReader
            ...
            If cbType.Items.Count = 0 Then
                If Not flg Then
                    ...
                    If UserLocked.Contains("AllowEstimateSales") Then
                        cmd.CommandText = "select DISTINCT Type,Descr,seq from SerialNum where Type>20 and Type<29 and Type <>25 order by seq"
                    Else
...
19:         dgInv.Columns.Add("PDisc", "P.Disc")
...
22:         dgInv.Columns.Add("Amount", "Amount")
            ...
            dgInv.Columns(19).DataPropertyName = "Pdisc"
            ...
            dgInv.Columns(19).SortMode = DataGridViewColumnSortMode.NotSortable
            ...
            dgInv.Columns.Item(19).HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight
            ...
            dgInv.Columns(19).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            ...
            Try
                cmd.CommandText = "select BillRateSelection from settings "
            ...
            cmd.CommandText = "select RndPaise,outgoid,ExpWiseBill,defaultDoc,DefaultPat,usebarcode,UseSalesmanPass,ShowPrateInSales,workshift,sysdt,ShowDis,FocusPack from settings"
            dt.Rows.Clear()
            dt.Clear()
            DA.SelectCommand = cmd
            DA.Fill(dt)
            ...
            For I = 0 To dt.Rows.Count - 1
                ...
                cbSal.Enabled = Not UseSalesmanPass
                FocusPack = dt.Rows(I).Item("FocusPack")
                If Not flg Then
                ...
                SysDt = dt.Rows(I).Item("sysdt")
                lbldt.Text = SysDt
                If dt.Rows(I).Item("ShowDis") = 0 Then
                    dgInv.Columns(19).Visible = False
                Else
                    dgInv.Columns(19).Visible = True
                End If
            Next
    ...
    Private Sub DgProdSer_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles DgProdSer.KeyDown
        If e.KeyCode = Keys.F10 Then
            If MsgBox("Add " & DgProdSer.Item(1, DgProdSer.CurrentRow.Index).Value & " to Short list ?", MsgBoxStyle.Question + MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton1) = MsgBoxResult.No Then Exit Sub
    ...
    Private Sub InvDetail()
        Try
            ...
            cmd.CommandText = "Select Batch.Stock - " & _
                ...
                "(OutGo.Batchid = Batch.BatchId)),0) AS STOCK,product.color,product.prdname,product.sched from Batch,product Where product.prdcode=batch.prdcode and  Batch.PrdCode=""" & DgProdSer.Item(0, DgProdSer.CurrentCell.RowIndex).Value & """ and ((batch.exdt IS NULL) or batch.exdt > convert(datetime,'" & SysDt & "')) "
            da.SelectCommand = cmd
            da.Fill(dt)
            tmpvar = 0
            pn = ""
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                sch = dt.Rows(i).Item("sched") & ""
            Next

            If tmpvar = 0 Then
                pnlPDet.Visible = False
                pnlprod.Visible = False
                If MsgBox(pn & Chr(13) & "No Stock..!" & Chr(13) & "Want to add to short list ? ", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton1) = MsgBoxResult.Yes Then
                    cmd.CommandType = CommandType.StoredProcedure
                    cmd.CommandText = "Update_ShortItem"
                    ...
                    BatchAdd.Tag = DgProdSer.Item(0, DgProdSer.CurrentCell.RowIndex).Value
                    BatchAdd.ClearAll()
                    BatchAdd.ShowDialog()
                    If BatchAdd.Tag <> "" Then
                        GoTo Cnt
                    End If
                Else
...
Cnt:
            If sch = "H1" Then
                'If MsgBox(" Scheduled H1" & Chr(13) & "Want to add for sales ? ", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2) = MsgBoxResult.Yes Then
                'Else
                '    pnlPDet.Visible = False
                '    pnlprod.Visible = False
                '    txtEdit.Focus()
    ...
    Private Sub AdjQty()
        ...
        Try
            ...
            If CbRate.SelectedIndex = 0 Then
                ...
                End If
            ElseIf CbRate.SelectedIndex = 1 Then
                dgInv.Item(14, dgInv.CurrentCell.RowIndex).Value = Format(dgInv.Item(3, dgInv.CurrentCell.RowIndex).Value * dgBDet.Item(2, dgBDet.CurrentCell.RowIndex).Value, "0.0000")
                dgInv.Item(16, dgInv.CurrentCell.RowIndex).Value = dgBDet.Item(14, dgBDet.CurrentCell.RowIndex).Value
                ...
                dgInv.Item(17, dgInv.CurrentCell.RowIndex).Value = "No"
                ' End If
            ElseIf CbRate.SelectedIndex = 2 Then
                dgInv.Item(14, dgInv.CurrentCell.RowIndex).Value = Format(dgInv.Item(3, dgInv.CurrentCell.RowIndex).Value * dgBDet.Item(13, dgBDet.CurrentCell.RowIndex).Value, "0.0000")
                dgInv.Item(16, dgInv.CurrentCell.RowIndex).Value = dgBDet.Item(14, dgBDet.CurrentCell.RowIndex).Value
            ...
            dgInv.Item(15, dgInv.CurrentCell.RowIndex).Value = "0.00" ',dgBDet.Item(8, dgBDet.CurrentCell.RowIndex).Value
            dgInv.Item(19, dgInv.CurrentCell.RowIndex).Value = 0
            ...
            If Val(dgInv.Item(16, dgInv.CurrentCell.RowIndex).Value & "") = 0.0 Then
                dgInv.Item(16, dgInv.CurrentCell.RowIndex).Style.BackColor = Color.SkyBlue
            Else
                dgInv.Item(16, dgInv.CurrentCell.RowIndex).Style.BackColor = Color.White
            End If
            ...
            If n <> 0 Then
                For i = 0 To dgBDet.Rows.Count - 1
                    If dgBDet.Item(15, i).Value <> dgBDet.Item(15, dgBDet.CurrentCell.RowIndex).Value Then
                        ...
                        ElseIf CbRate.SelectedIndex = 1 Then
                            dgInv.Item(14, ir).Value = Format(dgBDet.Item(2, i).Value, "0.0000")
                            dgInv.Item(17, ir).Value = "No"
                            dgInv.Item(16, ir).Value = dgBDet.Item(14, i).Value
                        ElseIf CbRate.SelectedIndex = 2 Then
                            dgInv.Item(14, ir).Value = Format(dgBDet.Item(13, i).Value, "0.0000")
                            dgInv.Item(17, ir).Value = "Yes"
                            dgInv.Item(16, ir).Value = dgBDet.Item(14, i).Value
                        End If

                        dgInv.Item(15, ir).Value = "0.00" ' dgBDet.Item(8, i).Value
                        dgInv.Item(19, ir).Value = 0
    ...
    Private Sub ShowEditControl(ByVal cntrol As Object)
        ...
        Try
            ...
            If cntrol.name = CbEditUnit.Name And dgInv.CurrentCell.Value & "" = "" Then
                If FocusPack > CbEditUnit.Items.Count Then
                    cntrol.selectedindex = CbEditUnit.Items.Count - 1
                Else
                    cntrol.selectedindex = (FocusPack - 1)
                End If
            Else
        ...
        Try
            ...
            Select Case dgInv.CurrentCell.ColumnIndex
                ...
                Case 16
                    ...
                    If dgInv.Item(7, dgInv.CurrentCell.RowIndex).Value = "12.50" Then
                        dgInv.CurrentCell.Style.BackColor = Color.Pink
                    Else
                        dgInv.CurrentCell.Style.BackColor = Color.White
                    End If
            ...
            If e.KeyCode = Keys.Enter Then
                ...
                Select Case dgInv.CurrentCell.ColumnIndex
                    ...
                    Case 14
                        ...
                        dgInv.Enabled = True
                    Case 19
                        dgInv.Item(dgInv.CurrentCell.ColumnIndex, dgInv.CurrentCell.RowIndex).Value = Val(txtEdit.Text)
                        dgInv.CurrentCell = dgInv.Item(19, dgInv.CurrentCell.RowIndex)
                        txtEdit.Visible = False
                        dgInv.Enabled = True
                    Case 15
    ...
    Private Sub GetBarcode()
        ...
        Try
            ...
            If cmd.ExecuteScalar = 0 And Microsoft.VisualBasic.Left(txtEdit.Text, 1) = "B" Then

                If Microsoft.VisualBasic.Left(txtEdit.Text, 1) <> "B" Then
                    bat = Microsoft.VisualBasic.Mid(txtEdit.Text, 1, 15)
                Else
                ...
                cmd.Connection = Conn
                cmd.CommandText = "Select batch.*,product.prdcode,product.prdname,product.case1,ISNULL(BATCH.UNAC,0) AS UNAC,NODISC from                                              batch,product  where product.prdcode=batch.prdcode and batchid=" + bat

                da.SelectCommand = cmd
                da.Fill(dt)
                If dt.Rows(0).Item("stock") = 0 Then
                    cmd.CommandText = "Select batch.*,product.prdcode,product.prdname,product.case1 from batch,product  where                               product.prdcode=batch.prdcode and product.prdcode=""" + dt.Rows(0).Item("prdcode") & """ and                                        batch.batch=""" & dt.Rows(0).Item("batch") & """ and stock<>0 "
                    dt.Rows.Clear()
                    dt.Clear()
                    da.Fill(dt)
                End If
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    dgInv.Item(21, dgInv.CurrentCell.RowIndex).Value = dt.Rows(0).Item("nodisc")
                    dgInv.Item(20, dgInv.CurrentCell.RowIndex).Value = IIf(IsDBNull(dt.Rows(0).Item("unac")), 0, dt.Rows(0).Item("unac"))
                    ...
                    dgInv.Item(19, dgInv.CurrentCell.RowIndex).Value = Round(dt.Rows(0).Item("Pdis"), 2)
    ...
    Private Sub GetPacking()
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                ' CbEditUnit.SelectedIndex = FocusPack
    ...
    Private Sub CbEditUnit_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles CbEditUnit.KeyDown
        ...
        Try
            ...
            If e.KeyCode = Keys.Enter Then
                ...
                If (FocusPack - 1) <> CbEditUnit.SelectedIndex And CbEditUnit.Items.Count > 1 Then
                    dgInv.CurrentCell.Style.BackColor = Color.Pink
                Else
                    dgInv.CurrentCell.Style.BackColor = Color.White
                End If
    ...
    Private Sub cbEditVat_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cbEditVat.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then
                ...
                If cbEditVat.Text = "0.00" And cbEditVat.Items.Count > 0 Then
                    dgInv.CurrentCell.Style.BackColor = Color.SkyBlue
                Else
                    dgInv.CurrentCell.Style.BackColor = Color.White
                End If
    ...
    Public Sub Calculate()
        ...
        Try
            ...
            For n = 0 To dgInv.Rows.Count - 2
                ...
                'prate = Val(dgInv.Item(14, n).Value)
                ...
                If stk <> 0 Then
                    ...
                    ' value = System.Math.Round((stk * value), 2)
                    scst = value * Val(dgInv.Item(15, n).Value) / 100
                    totcst = totcst + scst
                    value = value + scst

                    pdisc = value * Val(dgInv.Item(19, n).Value & "") / 100                 'Val(dgInv.Item(14, n).Value) - ((((Val(dgInv.Item(14, n).Value)) * Val(dgInv.Item(19, n).Value))) / 100)
                    value = value - pdisc
    ...
    Private Sub tsbSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbSave.Click
        ...
        If cbType.SelectedValue <> "24" Then
            cmd.Connection = Conn
            For i = 0 To dgInv.RowCount - 2

                cmd.CommandText = "select Sched from product where PRDcode=""" & dgInv.Rows(i).Cells(1).Value & """"
                If StrConv(cmd.ExecuteScalar & "", vbUpperCase) = "H1" Then
                    If cbPat.Text = "" Then
                        MsgBox("Schedule H1 Drug " & dgInv.Rows(i).Cells(7).Value & "." & Chr(13) & "Patient name is blank.")
                        cbPat.Focus()
                        Exit Sub
                    End If

                    If cbDoc.Text = "" Then
                        MsgBox("Schedule H1 Drug " & dgInv.Rows(i).Cells(7).Value & "." & Chr(13) & "Doctor name is blank.")
                        cbDoc.Focus()
                        Exit Sub
                    End If
                    If txtplace.Text = "" Then
                        MsgBox("Schedule H1 Drug " & dgInv.Rows(i).Cells(7).Value & "." & Chr(13) & "Patient address is blank.")
                        txtplace.Focus()
                        Exit Sub
                    End If
                End If
        ...
        Try
            ...
            sql = "insert into invtotal (Invno,Invdt,type,CusCode,Name," & _
                 "BDis,BDisAmt,TFAmt,Amt1," & _
                  ...
                  "node,Cashamt,cardamt,couamt,paydet,payref,empno,esino,empname,presno,cancelled,edited,sman,workshift,mobno,pl,unacamt,docadr,RrepAmt) Values(" & InvNum & _
                  ...
                  "," & Val(cbSal.SelectedValue & "") & "," & IIf(EditFlag, editworkshift, WorkShift) & ",""" & txtmobno.Text & """,""" & txtplace.Text & """," & unacamt & _
                  ",""" & txtDoc.Text & """," & Val(txtRecv.Text) & ")"
            ...
            For i = 0 To dgInv.Rows.Count - 2
                ...
                sql = "insert into invdetails(Invno,Invdt,Type,PrdCode,Batch,Batchid,ExDt,Pack,Qty," & _
                      "Srate,ST,cst,Unit,SValue,Pdis,Seq,Sman,Bdis,Priflag," & _
                      ...
                      "," & Val(dgInv.Item(19, i).Value) & "," & i & "," & Val(cbSal.SelectedValue & "") & _
            ...
            For i = 0 To dgRtn.Rows.Count - 2   'returns
                ...
                If dgRtn.Item(1, i).Value & "" <> "" Then
                    sql = "insert into invdetails(Invno,Invdt,Type,PrdCode,Batch,Batchid,ExDt,Pack,Qty," & _
                            ...
                            "," & Val(dgInv.Item(19, i).Value) & "," & seq & "," & Val(cbSal.SelectedValue & "") & _
    ...
    Private Sub tsbEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbEdit.Click
        Try
            ...
            If CheckUserLocked("Open Previous Sales", True) = False Then
                Me.Cursor = Cursors.Default
                If CheckUserLocked("Edit Sales", , dt.Rows(0).Item("invdt")) = False Then Exit Sub
            End If
    ...
    Private Sub EditingSales(ByVal no As String, ByVal dtf As Date, ByVal dtt As Date)
        Try
            ...
            EditDt = Nothing
            EditVouno = 0
            'p_disc = 100 - ((pdisc * 100) / prate)


            cmd.CommandText = "select invtotal.*,InvDetails.*,Product.PrdName,Product.nodisc,invtotal.rrepamt from invtotal," & _
            ...
            If dtab.Rows.Count <> 0 Then
                cmd.CommandText = "select invtotal.*,InvDetails.*,Product.PrdName,isnull(Product.nodisc,0) as nodisc,invtotal.rrepamt from invtotal," & _
                         "InvDetails,Product where Product.PrdCode=InvDetails.prdcode and " & _
                         "invtotal.InvNo=InvDetails.InvNo and invtotal.Invdt=InvDetails.Invdt and " & _
                         "invdetails.type=invtotal.type and InvDetails.InvNo=" & no & _
            ...
            If dtab.Rows.Count <> 0 Then
                ...
                For i = 0 To dtab.Rows.Count - 1
                    If slno = 0 Then
                        ...
                        ls = dtab.Rows(i).Item("DedAmt")
                        ad = dtab.Rows(i).Item("AddAmt")
                        ...
                        cmbpaydet.Text = dtab.Rows(i).Item("paydet") & ""
                        ...
                        txtDoc.Text = (dtab.Rows(i).Item("docadr") & "")
                        editworkshift = Val(dtab.Rows(i).Item("workshift") & "")
                        txtRecv.Text = Format(Val(dtab.Rows(i).Item("rrepamt") & ""), "0.00")
                    ...
                    Else
                        ...
                        dgInv.Item(19, dgInv.Rows.Count - 1).Value = Round(dtab.Rows(i).Item("Pdis"), 2)
                        ...
                        If IsDBNull(dtab.Rows(i).Item("UNAC")) Then
                            dgInv.Item(20, dgInv.Rows.Count - 1).Value = False
                        Else
            ...
            txtEdit.Visible = False
            Calculate()
            txtDed.Text = Format(ls, "0.00") ' dtab.Rows(i).Item("DedAmt")
            txtAdd.Text = Format(ad, "0.00") 'dtab.Rows(i).Item("AddAmt")
        Catch er As Exception
            ErrorMsg(er.Message, er.StackTrace)
            ClearForm()
        End Try
    ...
    Private Sub ClearRow()
        Try
            ...
            dgInv.Item(19, dgInv.CurrentCell.RowIndex).Value = Nothing
    ...
    Private Sub cbccard_SelectedIndexChanged1(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbccard.SelectedIndexChanged
        ...
        txtccard.Tag = ""
        LblCard.Text = "Card No: " & txtccard.Text + " (" + cbccard.Text + ")"
    ...
    Private Sub btnpresadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnpresadd.Click
        ...
        For x = 0 To dt1.Rows.Count - 1
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                dgInv.Item(19, dgInv.Rows.Count - 1).Value = "0.00"
                ...
                Else
                    dgInv.Item(17, dgInv.Rows.Count - 1).Value = "Yes"
                End If
                dgInv.Item(20, dgInv.Rows.Count - 1).Value = dt.Rows(i).Item("nodisc")
                dgInv.Item(21, dgInv.Rows.Count - 1).Value = dt.Rows(i).Item("unac")
                dgInv.Update()
                Sales.Rows.Add()
...
che:
        ...
        If CheckUserLocked("Open Previous Sales", True) = False Then
            Me.Cursor = Cursors.Default
            If CheckUserLocked("Edit Sales", , DT) = False Then Exit Sub
        End If
...
che:
        ...
        If histno <> 0 Then
            ...
            For v = 0 To dt1.Rows.Count - 1
                ...
                For i = 0 To dt.Rows.Count - 1
                    ...
                    dgInv.Item(19, dgInv.Rows.Count - 1).Value = Round(dt1.Rows(v).Item("Pdis"), 2)
    ...
    Private Sub FindCardDetails()
        ...
        Dim cc As String = ""
        cmd.Connection = Conn
        cmd.CommandText = "select convert(varchar,cardno) + ' ('+  name +')'  from carddetails where phone=""" & txtmobno.Text & """"
    ...
    Private Sub cbCus_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cbCus.SelectedIndexChanged
    ...
    Private Sub cbDoc_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cbDoc.SelectedIndexChanged
        Try
            ...
            txtDoc.Text = ""
            cmd.Connection = Conn
            cmd.CommandText = "select place from doctor where doccode=" & Val(cbDoc.SelectedValue.ToString & "")
    ...
    Private Sub CbRate_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles CbRate.SelectedIndexChanged
        Dim cmd As New OleDb.OleDbCommand
        Dim da As New OleDb.OleDbDataAdapter
        Dim dt As New DataTable
        cmd.Connection = Conn
        ...
        For i As Integer = 0 To dgInv.Rows.Count - 2
            cmd.CommandText = "select srate,stax,prate,ptax,trrate,taxinc from batch where batchid=" & Val(dgInv.Item(2, i).Value & "")
            da.Fill(dt)
            If dt.Rows.Count <> 0 Then
                If CbRate.SelectedIndex = 0 Then
                    dgInv.Item(14, i).Value = Format(dgInv.Item(3, i).Value * dt.Rows(0).Item("srate"), "0.0000")
                    dgInv.Item(16, i).Value = dt.Rows(0).Item("stax")
                    If dt.Rows(0).Item("TAXINC") Then
                        dgInv.Item(17, i).Value = "Yes"
                    ElseIf dgBDet.Item(10, i).Value = False Then
                        dgInv.Item(17, i).Value = "No"
                    End If
                ElseIf CbRate.SelectedIndex = 1 Then
                    dgInv.Item(14, i).Value = Format(dgInv.Item(3, i).Value * dt.Rows(0).Item("Prate"), "0.0000")
                    dgInv.Item(16, i).Value = dt.Rows(0).Item("ptax")
                    dgInv.Item(17, i).Value = "No"
                Else
                    dgInv.Item(14, i).Value = Format(dgInv.Item(3, i).Value * dt.Rows(0).Item("trrate"), "0.0000")
                    dgInv.Item(16, i).Value = dt.Rows(0).Item("stax")
                    dgInv.Item(17, i).Value = "Yes"
                End If
    ...
    Private Sub lblNamt_DoubleClick(sender As Object, e As System.EventArgs) Handles lblNamt.DoubleClick

        Select Case lblNamt.ForeColor
            Case Color.Blue
                lblNamt.ForeColor = Color.Red
            Case Color.Red
                lblNamt.ForeColor = Color.Yellow
            Case Color.Yellow
                lblNamt.ForeColor = Color.White
            Case Color.White
                lblNamt.ForeColor = Color.Blue
    ...
    Private Sub Button8_Click(sender As System.Object, e As System.EventArgs) Handles BtnImg.Click
        ImageFrm.ShowDialog()
    End Sub
